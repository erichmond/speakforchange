...
function PagePlayer(oConfigOverride) {
  ...
  this.events = {
    ...
    play: function() {
      ...
      this._data.oLink.innerHTML = "Pause message";

      self.setPageTitle(this._data.originalTitle);

      // patch dhchoi
      $(".currentlyPlayingMessage").removeClass("currentlyPlayingMessage");
      self.addClass(this._data.oLI.parentNode.parentNode,"currentlyPlayingMessage"); // add default CSS decoration
    ...
    stop: function() {
      ...
      this._data.oLink.innerHTML = "Play message";
    ...
    finish: function() {
      ...
      // patch by danchoi. Don't play any non-visible li; but this only applies
      // if the currentZipcode variable is set (by big_map.js)
      var nextMessage;
      if (typeof currentZipcode != 'undefined') {
      
        nextMessage = $('.currentlyPlayingMessage').nextAll('.zipcode_' + currentZipcode)[0]; 
      } else {
        nextMessage = $('.currentlyPlayingMessage').nextAll()[0]; 
      }

      //console.log(nextMessage);
      // play next if next message
      // if (self.config.playNext && this._data.nIndex<pl.links.length-1) {

      if (self.config.playNext && nextMessage) {
        var nextAudio = $(nextMessage).find('a.audio')[0];
        //console.log("test");
        //console.log(nextAudio);
        //
        //pl.handleClick({target:pl.links[this._data.nIndex+1]}); // fake a click event - aren't we sneaky. ;)
        pl.handleClick({target:nextAudio}); // fake a click event - aren't we sneaky. ;)
      } else {
        self.setPageTitle();
        self.resetPageIcon();
      }
  ...
  this.updateTime = function() {
    // patch speakforchange
    // If seconds > x, mark as scrobbled.
    // 
    var seconds = parseInt(this.position / 1000);
    
    if ( seconds >= 1 && !this.scrobbled) {
      var messageDiv  = this._data.oLink.parentNode.parentNode.parentNode; 
      var messageId = messageDiv.id.split("_")[1]; 

      this.scrobbled = true; // scrobble only once per audio file
      jQuery.post("/messages/" + messageId + "/plays"); 
    }
    // end of patch
